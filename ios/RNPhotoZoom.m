...
- (void)view:(UIView *)view singleTapDetected:(UITouch *)touch {
    ...
    if (_onPhotoZoomerViewTap) {
        _onPhotoZoomerViewTap(@{
                                @"point": @{
                                        @"x": @(touchX),
                                        @"y": @(touchY),
                                        },
        ...
        if (![[uri substringToIndex:4] isEqualToString:@"http"]) {
            @try {
                ...
                if (image) { // if local image
                    [self setImage:image];
                    if (_onPhotoZoomerLoad) {
                        _onPhotoZoomerLoad(nil);
                    }
                    if (_onPhotoZoomerLoadEnd) {
                        _onPhotoZoomerLoadEnd(nil);
                    }
                    return;
                }
            }
...
//            request = [mutableRequest copy];
        ...
        __weak RNPhotoZoom *weakSelf = self;
        if (_onPhotoZoomerLoadStart) {
            _onPhotoZoomerLoadStart(nil);
        }
        ...
        [mgr loadImageWithURL: imageURL
                     ...
                     progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
                        if (_onPhotoZoomerProgress) {
                            _onPhotoZoomerProgress(@{
                    ...
                    completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, SDImageCacheType cacheType, BOOL finished, NSURL * _Nullable imageURL) {
                        if (image) {
                            ...
                            if (_onPhotoZoomerError) {
                                _onPhotoZoomerError(nil);
                            }
                        }
                        if (_onPhotoZoomerLoadEnd) {
                            _onPhotoZoomerLoadEnd(nil);
